...
\begin{table}[p]
    \centering
    \scriptsize
    \caption[Riassunto della verifica delle tensioni massime di esercizio in combinazione quasi permanente]{Riassunto della verifica delle tensioni massime di esercizio in combinazione quasi permanente: $\sigma_{cR} = \SI{11.25}{\mega\pascal}$ e $\sigma_{sR} = \sigma_{sR}^\prime = \SI{360.00}{\mega\pascal}$}
...
In accordo anche a quanto mostrato schematicamente in figura \ref{fig:sle_deformazione}, si vengono a creare 8 diverse zone, occorre perciò integrare in ognuna di essi con diversi valori degli estremi (coordinata $\xi$).
\begin{figure}[htb]
    \centering
...
Per ogni tratto $j-$esimo la deformazione (coincidente con il lavoro virtuale essendo la forza unitaria) la si calcola come:
\begin{equation}
...
Con le equazioni sopra esposte, suddividendo la campata nelle seguenti coordinate: 
\[
    \begin{bmatrix}
...
Al \normaref{\S C4.1.4} è spiegato come effettuare la verifica semplificata nel caso di luci inferiori ai \SI{10}{\metre}.
